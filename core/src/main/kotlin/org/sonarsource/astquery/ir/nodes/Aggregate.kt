...
class Aggregate<IN, OUT>(
  parent: ParentNode<IN>,
  val transform: IdentifiedFunction<(List<IN>) -> OUT>
) : IRNode<IN, OUT>(parent) {

  override val isSink = false

  override fun canMergeWith(other: IRNode<*, *>) =
    other is Aggregate<*, *> && other.transform == transform

  override fun copy() = Aggregate(parents.single(), transform)
