...
class MapNode<IN, OUT>(
  id: NodeId,
  children: List<ChildNode<OUT>>,
  private val mapper: (IN) -> OUT,
) : BatchNode<IN, OUT>(id, children) {

  override val isSink = false

  override fun onValue(context: ExecutionContext, caller: NodeId, value: Signal.Value<IN>) {
    propagate(context, value.apply { values -> values.map(mapper) })
  }

  override fun getFlowType(parentsInfo: Map<BatchNode<*, *>, VisualInfo>): FlowType = getParentFlowType(parentsInfo)
