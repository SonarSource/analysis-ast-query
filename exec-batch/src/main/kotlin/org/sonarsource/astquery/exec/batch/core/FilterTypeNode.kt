...
class FilterTypeNode<IN, OUT : IN & Any>(
  ...
  override fun onValue(context: ExecutionContext, caller: NodeId, value: Signal.Value<IN>) {
    val input = value.values
    val filtered = input.filter { types.any { type -> type.isInstance(it) } }

    @Suppress("UNCHECKED_CAST")
    val cast = filtered.map { it as OUT }
    propagate(context, Signal.Value(cast, value.scopes))
  }
