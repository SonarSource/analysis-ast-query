...
class FirstNode<IN>(
  ...
  override fun onValue(context: ExecutionContext, caller: NodeId, value: Signal.Value<IN>) {
    val first = value.apply { values -> values.firstOrNull()?.let(::listOf) ?: emptyList() }
    propagate(context, first)
  }

  override fun getFlowType(parentsInfo: Map<BatchNode<*, *>, VisualInfo>) = FlowType.Opt

  override fun toString() = "First-$id"
}
