...
class FilterNonNullNode<IN>(
  ...
  override fun onValue(context: ExecutionContext, caller: NodeId, value: Signal.Value<IN?>) {
    propagate(context, value.apply { values -> values.filterNotNull() })
  }

  override fun getFlowType(parentsInfo: Map<BatchNode<*, *>, VisualInfo>) =
    if (getParentFlowType(parentsInfo) == FlowType.Many) FlowType.Many else FlowType.Single

  override fun toString() = "FilterNonNull-$id"
}
